...
function renderRanking(playerName) {
    ...
    const playerScores = ranking.filter(player => player.name === playerName);
    const playerPosition = ranking.findIndex(player => player.name === playerName && player.score === score.value);
    ...
    if (playerPosition >= 0) {
        ...
        // Se o jogador não estiver no ranking, adicione sua posição baseada na sua pontuação
        const realPosition = playerScores.length > 0 ? playerScores.length + 1 : ranking.length + 1; // se não estiver no ranking
